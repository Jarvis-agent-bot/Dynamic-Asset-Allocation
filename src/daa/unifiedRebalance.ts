...
function normalizeStance(v: unknown): DaaAnalystStance {
  if (v === "offensive") return "offensive";
  if (v === "defensive") return "defensive";
  return "neutral";
}

function normalizeMomentum(v: unknown): DaaMomentumRegime {
  if (v === "strong") return "strong";
  if (v === "weak") return "weak";
  return "neutral";
}

function normalizeRiskTierFromTags(tags: string[]): DaaRiskTier {
...
function normalizeCcyCode(v: unknown, fallback = "USD"): string {
  return normalizeDaaCurrencyCode(v, fallback);
}
...
function buildFxLookup(fxRates: DaaUnifiedFxRate[]): Map<string, FxLookupValue> {
  const map = new Map<string, FxLookupValue>();
...
function resolveLocalToBaseRate(
  fxMap: Map<string, FxLookupValue>,
  localCcy: string,
  baseCcy: string,
...
function collectCrossMarketExposure(positions: DaaUnifiedPosition[]): Record<string, number> {
  const out: Record<string, number> = {};
...
export function isDaaUnifiedRequest(x: unknown): x is DaaUnifiedRequest {
  ...
  for (const [rawKey, rawWeight] of Object.entries(targetWeights)) {
    ...
    if (!parseDaaAssetKey(keyText)) return false;
    const weight = Number(rawWeight);
...
export function buildDaaUnifiedPlan(req: DaaUnifiedRequest): DaaUnifiedResponse {
  ...
  const assetMetaByKey = new Map<string, DaaAssetMeta>();
  const assetKeysBySymbol = new Map<string, string[]>();
  const riskTierBySymbol = new Map<string, DaaRiskTier>();
  const riskTierBudget: Record<DaaRiskTier, number> = { low: 0, mid: 0, high: 0 };

  const appendAssetMeta = (meta: DaaAssetMeta) => {
  ...
  for (const raw of req.candidateAssets ?? []) {
    ...
    if (!symbol) continue;
    const market = normalizeDaaMarket(raw.market, "US");
  ...
  for (const assetKey of Object.keys(inputTargetWeightsByAssetKey)) {
    ...
    const parsedAssetKey = parseDaaAssetKey(assetKey);
  ...
  const analystMap = new Map<string, { scorePct: number; stance: DaaAnalystStance; styleCluster: string }>();
  ...
  const viewsBySymbol = new Map<string, DaaUnifiedAssetView[]>();
  ...
  const assetDecisions: DaaHumanFactorDecision[] = [];
  ...
  for (const [symbol, baseWeight] of Object.entries(targetWeights)) {
    ...
    let tier: DaaHumanFactorDecision["tier"] = "steady";
    ...
    const momentumRegime: DaaMomentumRegime = strongMomentum ? "strong" : "neutral";
  ...
  const executableOrders: DaaExecutableOrder[] = [];
  const blockedOrders: DaaBlockedOrder[] = [];
  ...
  for (const order of core.orders) {
    ...
    const parsedOrderKey = parseDaaAssetKey(rawAssetKey);
    if (!parsedOrderKey) {
      throw new Error(`order assetKey is invalid: ${rawAssetKey}`);
    }
    const normalizedAssetKey = buildDaaAssetKey(parsedOrderKey.symbol, parsedOrderKey.market);
...
export function buildDaaUnifiedPlanFromUnknown(raw: unknown): DaaUnifiedResponse {
  if (!isDaaUnifiedRequest(raw)) {
    throw new Error("invalid request shape");
  }
  return buildDaaUnifiedPlan(raw);
}

export const DAA_UNIFIED_SAMPLE_REQUEST_: DaaUnifiedRequest = {
  account: {
    baseCurrency: "USD",
    cash: 12000,
  },
