...
function parseBrainModeSwitch(text: string): DaaAssistantIntent | null {
  const advisorPattern = /(切(换)?到|改成|切成|设为|设置为|变成|进入).*(仅建议授权|仅建议|顾问模式|顾问|advisor)/i;
  const operatorPattern = /(切(换)?到|改成|切成|设为|设置为|变成|进入).*(手动复核授权|手动复核|操作员模式|操作员|operator)/i;
  const autopilotPattern = /(切(换)?到|改成|切成|设为|设置为|变成|进入).*(自动复核授权|自动复核|自动驾驶模式|自动驾驶|autopilot)/i;
  ...
  if (/(开启|打开).*(自动复核|自动驾驶|autopilot)/i.test(text)) return buildIntent("autopilot");
  if (/(关闭|退出).*(自动复核|自动驾驶|autopilot)/i.test(text)) return buildIntent("operator");
  return null;
}
...
export function parseAssistantIntent(raw: string, options?: {
  ...
  if (/(投资助理状态|投资助理能力|投资复核链路|全权大脑|大脑状态|系统能力|权限边界|认知链路|模型路由|接入.*模型|现在能做什么|brain status|capability|分级授权|授权等级|仅建议|手动复核|自动复核|顾问模式|操作员模式|自动驾驶模式)/i.test(text)) {
  ...
  if (/(初始化|创建初始).*?(投资判断|论点|thesis)|bootstrap/i.test(text)) {
    return downgradeExecutionIntent({ kind: "agent_bootstrap", rawText: text }, allowExecution);
  }
  if (/(运行|启动|执行|刷新).*(投资复核|投资助理|agent|认知|调查|日报)|跑一轮.*(投资复核|投资助理|agent|认知|调查)/i.test(text)) {
    return { kind: "agent_run", rawText: text };
  }
  if (/投资判断|论点|thesis|研究线索|theses|conviction/i.test(text)) return { kind: "thesis_status", rawText: text };
  if (/复核简报|投资助理简报|简报|日报|briefing|认知缺口|自动跟踪|意外|改观条件|复核变化|需要复核|改变判断|agent.*报/i.test(text)) return { kind: "agent_briefing", rawText: text };
