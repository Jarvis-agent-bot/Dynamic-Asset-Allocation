...
function buildPlannerPrompt(input: AssistantPlanningInput): string {
  return [
    ...
    "你的第一任务是判断当前用户输入最适合映射到哪一个动作；当用户在问系统能力、模型路由、权限边界、投资复核链路时，可直接基于给定上下文回答。",
    ...
    "5. 用户在问当前投资助理能力、模型路由、系统权限、投资复核链路，用 brain_status。",
    "6. 用户要求切到仅建议 / 手动复核 / 自动复核授权，或使用旧说法顾问 / 操作员 / 自动驾驶模式，用 brain_set_mode。",
    "7. 用户要求运行 / 启动 / 刷新一轮投资复核，用 agent_run。",
    "8. 用户要求建立初始投资判断，或使用初始化 / bootstrap thesis 旧说法，用 agent_bootstrap。",
    ...
    "14. 用户要求看活跃投资判断、复核线索、conviction，用 thesis_status。",
    "15. 用户要求看复核简报、投资助理简报、投资判断复核、需要复核的变化、改变判断的条件，用 agent_briefing。",
