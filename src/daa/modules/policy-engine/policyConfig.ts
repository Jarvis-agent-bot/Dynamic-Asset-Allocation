...
export function resolvePolicyConfig(config: DaaSystemConfig): DaaPolicyConfig {
  ...
  const outerBandPct = Math.max(
    ...
    source.drift.outerBandPct || fallback.drift.outerBandPct,
  );
  ...
  return {
    ...
    review: {
      ...
      dayOfMonth: Math.max(1, Math.min(28, Math.trunc(source.review.dayOfMonth || fallback.review.dayOfMonth))),
    ...
    throttle: {
      proposalDedupeWindowHours: Math.max(
        1,
        Math.trunc(source.throttle.proposalDedupeWindowHours || fallback.throttle.proposalDedupeWindowHours),
      ),
      autoExecutionCooldownHours: Math.max(1, Math.trunc(source.throttle.autoExecutionCooldownHours || fallback.throttle.autoExecutionCooldownHours)),
    ...
    execution: {
      ...
      maxSingleOrderPctOfNav: Math.max(
        0.01,
        Math.min(0.5, source.execution.maxSingleOrderPctOfNav || fallback.execution.maxSingleOrderPctOfNav),
      ),
    },
  };
}
