import {
  ...
  appendDaaMarketPriceHistoryRowsV1,
  deleteExpiredDaaExternalPayloadRawV1,
  getDaaMarketCacheHealthStatsV1,
  listDaaMarketPriceSnapshotsV1,
  listLatestDaaMarketPriceHistoryRowsV1,
  upsertDaaMarketPriceSnapshotsV1,
...
export type MarketCachePriceStatusV1 = "fresh" | "stale" | "missing";

export type MarketPriceAssetInputV1 = {
  symbol: string;
  market: string;
  currency?: string;
};

export type MarketPriceResolvedV1 = {
  provider: string;
  symbol: string;
  market: string;
  currency: string;
  price: number;
  priceStatus: MarketCachePriceStatusV1;
  priceUpdatedAt: string | null;
  priceAgeSec: number | null;
  priceSource: string;
};

const DEFAULT_PROVIDER_V1 = "yfinance";
const DEFAULT_TIMEOUT_MS_V1 = 2600;
const DEFAULT_CONCURRENCY_V1 = 6;
const DEFAULT_REFRESH_BUDGET_V1 = 10;
const DEFAULT_FRESH_SEC_V1 = 15 * 60;
const DEFAULT_SERVE_STALE_SEC_V1 = 48 * 60 * 60;
...
function classifyByAge(ageSec: number | null, opts: { freshSec: number; serveStaleSec: number }): MarketCachePriceStatusV1 {
...
function resolveSnapshotStatusV1(snapshot: SnapshotLikeV1 | null | undefined, opts: { freshSec: number; serveStaleSec: number }): MarketCachePriceStatusV1 {
...
function resolveHistoryFallbackV1(input: {
  latestHistory: {
    price: number;
    ts: string;
    currency: string;
...
function pickLatestClose(payload: YfinanceChartPayloadV1 | null): number {
...
async function fetchYfinanceLatestCloseWithRawV1(symbol: string, timeoutMs: number): Promise<FetchResultV1> {
  ...
  try {
    ...
    const typedPayload = payloadJson as YfinanceChartPayloadV1 | null;
...
async function fetchWithRetryV1(symbol: string, timeoutMs: number): Promise<FetchResultV1> {
  const first = await fetchYfinanceLatestCloseWithRawV1(symbol, timeoutMs);
  if (first.ok || !first.retryable) return first;
  await sleep(350);
  return fetchYfinanceLatestCloseWithRawV1(symbol, timeoutMs);
}

export async function getMarketPricesWithCacheV1(input: {
  assets: MarketPriceAssetInputV1[];
  provider?: string;
  ...
  concurrency?: number;
}): Promise<Record<string, MarketPriceResolvedV1>> {
  const assets = Array.isArray(input.assets) ? input.assets : [];
  const provider = normalizeText(input.provider, DEFAULT_PROVIDER_V1);
  const allowRefresh = input.allowRefresh !== false;
  const forceRefresh = input.forceRefresh === true;
  const refreshBudget = Math.max(0, Math.min(200, Math.trunc(toFinite(input.refreshBudget, DEFAULT_REFRESH_BUDGET_V1))));
  const timeoutMs = Math.max(600, Math.min(8000, Math.trunc(toFinite(input.timeoutMs, DEFAULT_TIMEOUT_MS_V1))));
  const source = normalizeText(input.source, "market_cache");
  const freshSec = Math.max(60, Math.min(2 * 3600, Math.trunc(toFinite(input.freshSec, DEFAULT_FRESH_SEC_V1))));
  const serveStaleSec = Math.max(freshSec, Math.min(7 * 24 * 3600, Math.trunc(toFinite(input.serveStaleSec, DEFAULT_SERVE_STALE_SEC_V1))));
  const rawRetentionDays = Math.max(7, Math.min(365, Math.trunc(toFinite(input.rawRetentionDays, 90))));
  const concurrency = Math.max(1, Math.min(12, Math.trunc(toFinite(input.concurrency, DEFAULT_CONCURRENCY_V1))));

  const deduped = new Map<string, MarketPriceAssetInputV1>();
  ...
  const refreshTargets: MarketPriceAssetInputV1[] = [];
  if (allowRefresh) {
  ...
  if (refreshTargets.length > 0) {
    ...
    async function worker() {
      for (;;) {
        ...
        const fallbackFromHistory = resolveHistoryFallbackV1({
          latestHistory,
        ...
        if (!yfinanceSymbol) {
          ...
          const saved = await upsertDaaMarketPriceSnapshotsV1([
            {
              provider,
        ...
        const fetchResult = await fetchWithRetryV1(yfinanceSymbol, timeoutMs);
        ...
        if (fetchResult.ok && fetchResult.price > 0) {
          ...
          if (saved[0]) snapshotByKey.set(key, saved[0]);
          await appendDaaMarketPriceHistoryRowsV1([
            {
              provider,
              market: current.market,
              symbol: current.symbol,
  ...
  const out: Record<string, MarketPriceResolvedV1> = {};
  for (const row of rows) {
    const key = buildKey(row);
    const snapshot = snapshotByKey.get(key);
    ...
    const latestHistory = latestHistoryByKey.get(key) || null;
    const historyFallback = resolveHistoryFallbackV1({
      latestHistory,
      defaultCurrency,
    ...
    const status = resolveSnapshotStatusV1(snapshot, { freshSec, serveStaleSec });
...
export async function refreshMarketPricesV1(input: {
  assets: MarketPriceAssetInputV1[];
  ...
  stale: number;
  missing: number;
  results: Record<string, MarketPriceResolvedV1>;
}> {
  const startedAt = new Date().toISOString();
  const results = await getMarketPricesWithCacheV1({
    assets: input.assets,
  ...
  await appendDaaIngestJobLogV1({
    ...
    diagnosticsJson: {
      stale,
      missing,
      provider: normalizeText(input.provider, DEFAULT_PROVIDER_V1),
    },
  });
...
export async function getMarketCacheHealthV1(provider = DEFAULT_PROVIDER_V1) {
  return getDaaMarketCacheHealthStatsV1(provider);
}

export async function cleanupMarketCacheRawPayloadV1(nowIso?: string): Promise<{ removed: number; at: string }> {
  const at = nowIso ? new Date(nowIso).toISOString() : new Date().toISOString();
  const removed = await deleteExpiredDaaExternalPayloadRawV1(at);
  await appendDaaIngestJobLogV1({
