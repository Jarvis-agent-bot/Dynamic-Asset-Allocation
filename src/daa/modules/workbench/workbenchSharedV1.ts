...
import { buildMarketContextAttributionV1, getCurrentMarketContextV1, marketRegimeLabelZhV1, } from "@/src/daa/modules/marketContext/marketIndicatorServiceV1";
import { classifyCashV2 } from "./cashClassificationV2";
import { fuseDecisionV2 } from "./decisionFusionV2";
import { appendDaaTriggerEventV1, appendDaaRunHistoryV1, appendAssetPriceHistoryRowsV1, createDaaRebalanceCycleV1, createDaaRebalanceDecisionV1, createDaaTradeTicketV1, executeDaaTradeTicketsV1, getDaaCycleReportV1, getDaaHumanIngestStateV1, getDaaRebalanceCycleV1, getDaaSystemConfigV2, getDaaMarketCacheHealthStatsV1, listDaaAssetUniverseV1, listDaaCycleReportsV1, listDaaEquitySnapshotsV1, listDaaFxRatesV1, listDaaRebalanceCyclesV1, listDaaTradeTicketsV1, patchDaaRebalanceCycleV1, upsertDaaCycleReportV1, updateDaaAssetUniverseLastPriceV1, type DaaStoreRebalanceCycleV1, } from "@/src/daa/store/daaStorePgV1";
...
import type { ExecuteRebalanceSummaryV1, ExecuteRebalanceCycleResultV1, GenerateRebalanceCycleInputV1, GenerateRebalanceCycleResultV1, HfSignalSummaryV1, PortfolioHealthyInsightV1, PreTradeRiskCheckItemV1, PreTradeRiskCheckV1, RebalanceCycleV1, RebalanceProposalV1, RebalanceTriggerSourceV1, UpdateRebalanceCycleInputV1, WorkbenchBootstrapV1, WorkbenchRebalanceCycleReportV1, WorkbenchRecommendationV1, WorkbenchRecommendationsResultV1, WorkbenchTradeRecordsV1, } from "./workbenchTypesV1";
import { WorkbenchDomainErrorV1, type WorkbenchDomainErrorCodeV1 } from "./workbenchErrorsV1";
...
function getZonedYmdV1(date: Date, timeZone: string): {
    year: number;
    month: number;
    day: number;
} {
...
function isCalendarMonthDueV1(month: number, frequency: "monthly" | "quarterly" | "semi_annual" | "annual"): boolean {
...
async function buildHfSignalMapV1(): Promise<Map<string, HfSignalSummaryV1>> {
    ...
    for (const itemRaw of signals) {
        ...
        const funds = (holdingsBySymbol.get(symbol) || [])
            ...
            .slice(0, 6);
        map.set(symbol, buildHfSignalSummaryV1({
            symbol,
...
export function assertCycleMutableV1(cycle: {
    cycleId: string;
    status: RebalanceCycleV1["status"] | DaaStoreRebalanceCycleV1["status"];
}) {
    if (!isCycleTerminalV1(cycle.status))
        return;
    throw new WorkbenchDomainErrorV1("CYCLE_IMMUTABLE", "该周期已终态，请生成新周期继续调仓。", {
        details: {
...
export function assertCycleExecutableV1(cycle: {
    cycleId: string;
    status: RebalanceCycleV1["status"] | DaaStoreRebalanceCycleV1["status"];
}, actionLabel: "execute" | "summary") {
    if (isCycleExecutableV1(cycle.status))
        return;
    ...
    throw new WorkbenchDomainErrorV1(code, message, {
        details: {
            cycleId: cycle.cycleId,
            cycleStatus: cycle.status,
        },
...
export {
  ...
  buildTriggerEventIdempotencyKeyV1,
  appendTriggerEventSafeV1,
  calcHoldingCostPerUnitV1,
  buildRiskCycleDraftV1,
  calcPortfolioHhiPctV1,
  calcMaxWeightPctV1,
  calcMaxDriftPctV1,
  calcMaxDrawdownPctV1,
