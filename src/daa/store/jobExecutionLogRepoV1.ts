...
export type DaaJobExecutionLogV1 = {
  jobId: string;
  jobType: string;
...
function parseJsonObjectV1(value: unknown): Record<string, unknown> | null {
...
function mapJobLogRowV1(row: Record<string, unknown>): DaaJobExecutionLogV1 {
  return {
    ...
    resultJson: parseJsonObjectV1(row.result_json),
    errorText: row.error_text == null ? null : normalizeText(row.error_text) || null,
    createdAt: toIsoString(row.created_at),
  };
}

export async function appendJobExecutionLogV1(input: {
  jobId?: string;
  jobType: string;
  requestId?: string | null;
  triggerSource: string;
  ...
  return withDaaPgClientV0(async ({ query }) => {
    ...
    return mapJobLogRowV1(result.rows[0] as Record<string, unknown>);
  });
}

export async function listJobExecutionLogsV1(limit = 50): Promise<DaaJobExecutionLogV1[]> {
  await ensureDaaStoreSchemaPgV1();
  ...
  return withDaaPgClientV0(async ({ query }) => {
    ...
    return result.rows.map((row) => mapJobLogRowV1(row as Record<string, unknown>));
  });
}
