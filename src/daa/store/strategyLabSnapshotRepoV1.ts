...
export type DaaStrategyLabRunSnapshotV1 = {
  runId: string;
  createdAt: string;
  baseCurrency: string;
  startDate: string;
...
function mapSnapshotRowV1(row: Record<string, unknown>): DaaStrategyLabRunSnapshotV1 {
  return {
    ...
    requestJson: parseJsonObjectV1(row.request_json),
    summaryJson: parseJsonObjectV1(row.summary_json),
  };
}

export async function appendStrategyLabRunSnapshotV1(input: {
  runId?: string;
  createdAt?: string;
  baseCurrency: string;
  startDate: string;
  ...
  requestJson?: Record<string, unknown>;
  summaryJson?: Record<string, unknown>;
}): Promise<DaaStrategyLabRunSnapshotV1> {
  ...
  return withDaaPgClientV0(async ({ query }) => {
    ...
    return mapSnapshotRowV1(result.rows[0] as Record<string, unknown>);
  });
}

export async function listStrategyLabRunSnapshotsV1(limit = 20): Promise<DaaStrategyLabRunSnapshotV1[]> {
  await ensureDaaStoreSchemaPgV1();
  ...
  return withDaaPgClientV0(async ({ query }) => {
    ...
    return result.rows.map((row) => mapSnapshotRowV1(row as Record<string, unknown>));
  });
}
