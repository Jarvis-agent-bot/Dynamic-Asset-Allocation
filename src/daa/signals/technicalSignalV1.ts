...
export type DaaTechnicalSpecificMetricV1 = {
  key: string;
  label: string;
  value: number | string;
  unit?: string;
...
export type DaaTechnicalSignalV1 = {
  ...
  specific: DaaTechnicalSpecificMetricV1[];
  reasons: string[];
};
...
async function buildSpecificMetricsV1(symbol: string): Promise<DaaTechnicalSpecificMetricV1[]> {
  const normalized = String(symbol || "").trim().toUpperCase();

  if (/BTC/.test(normalized)) {
    const [btcCloses, ethCloses] = await Promise.all([
      fetchDailyClosesV1("BTC-USD", 120),
      fetchDailyClosesV1("ETH-USD", 120),
    ]);
  ...
  if (/XAU|GOLD|GC=F|XAG|SILVER|SI=F/.test(normalized)) {
    const [goldCloses, silverCloses] = await Promise.all([
      fetchDailyClosesV1("GC=F", 120),
      fetchDailyClosesV1("SI=F", 120),
    ]);
...
export async function buildTechnicalSignalForSymbolV1(symbol: string): Promise<DaaTechnicalSignalV1 | null> {
  const closes = await fetchDailyClosesV1(symbol, 260);
  ...
  const specific = await buildSpecificMetricsV1(symbol);
