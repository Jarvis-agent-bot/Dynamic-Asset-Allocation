...
export type DaaValuationMetricStatusV1 = "bullish" | "bearish" | "neutral" | "unavailable";

export type DaaValuationMetricV1 = {
  key: string;
  label: string;
  value: number | string;
  unit?: string;
  status?: DaaValuationMetricStatusV1;
  description?: string;
};

export type DaaValuationRelativeValueV1 = {
  key: string;
  label: string;
  value: number | null;
...
export type DaaValuationSignalV1 = {
  symbol: string;
  scorePct: number;
  ...
  relative: DaaValuationRelativeValueV1 | null;
  reasons: string[];
  specific: DaaValuationMetricV1[];
};

type FundamentalStatsV1 = {
  pe: number | null;
  pb: number | null;
  dividendYieldPct: number | null;
};
...
async function fetchDailyClosesV1(symbolRaw: string, days = 320): Promise<number[]> {
...
async function fetchFundamentalsV1(symbolRaw: string): Promise<FundamentalStatsV1> {
  const symbol = normalizeYfinanceSymbol(symbolRaw);
...
function inferAssetProfileV1(symbol: string): "crypto" | "commodity" | "default" {
...
async function buildRelativeValueV1(input: {
  symbol: string;
  getCloses: (symbol: string, days?: number) => Promise<number[]>;
}): Promise<{ metric: DaaValuationRelativeValueV1 | null; scoreDelta: number; reason: string | null }> {
...
function scoreTemperatureV1(score: number): "cheap" | "neutral" | "expensive" {
  if (score >= 62) return "cheap";
  if (score <= 38) return "expensive";
  return "neutral";
}

export async function buildValuationSignalForSymbolV1(
  symbolRaw: string,
  options?: {
    getCloses?: (symbol: string, days?: number) => Promise<number[]>;
  },
): Promise<DaaValuationSignalV1 | null> {
  const symbol = String(symbolRaw || "").trim().toUpperCase();
  if (!symbol) return null;

  const getCloses = options?.getCloses || fetchDailyClosesV1;
  ...
  let fundamentals: FundamentalStatsV1 = { pe: null, pb: null, dividendYieldPct: null };
  const profile = inferAssetProfileV1(symbol);
  if (profile !== "crypto") {
    fundamentals = await fetchFundamentalsV1(symbol);
  }

  const relative = await buildRelativeValueV1({
    symbol,
    getCloses,
  });
  ...
  const specific: DaaValuationMetricV1[] = [
    {
      key: "percentile90",
      label: "90日价格分位",
  ...
  return {
    ...
    temperature: scoreTemperatureV1(scorePct),
    metrics: {
...
export async function buildValuationSignalsV1(symbols: string[]): Promise<DaaValuationSignalV1[]> {
  ...
  const getCloses = (symbol: string, days = 320) => {
    ...
    if (!closesCache.has(key)) {
      closesCache.set(key, fetchDailyClosesV1(symbol, days));
    }
    return closesCache.get(key)!;
  };

  const rows = await Promise.all(cleanSymbols.map((symbol) => buildValuationSignalForSymbolV1(symbol, { getCloses })));
  return rows.filter((item): item is DaaValuationSignalV1 => Boolean(item));
}
