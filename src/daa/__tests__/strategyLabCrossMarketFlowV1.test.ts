...
describe("strategy-lab-cross-market-flow-v1", () => {
  ...
  it("支持加 HK/CN 资产 -> 跨币种回测 -> 写回当前配置", async () => {
    ...
    const bootstrapBefore = await buildWorkbenchBootstrapV1({ syncPrices: false, autoRiskCycle: false });
    ...
    const bootstrapAfter = await buildWorkbenchBootstrapV1({ syncPrices: false, autoRiskCycle: false });
