...
vi.mock("@/src/daa/store/daaStorePgV1", () => ({
  appendDaaExternalPayloadRawV1: vi.fn(async () => ({ id: "raw_test_1" })),
  appendDaaIngestJobLogV1: vi.fn(async () => ({ jobId: "job_test_1" })),
  appendDaaMarketPriceHistoryRowsV1: vi.fn(async () => 0),
  deleteExpiredDaaExternalPayloadRawV1: vi.fn(async () => 0),
  getDaaMarketCacheHealthStatsV1: vi.fn(async () => ({
    provider: "yfinance",
    totalSnapshots: 0,
    freshCount: 0,
    staleCount: 0,
  ...
  listDaaMarketPriceSnapshotsV1: vi.fn(async () => []),
  listLatestDaaMarketPriceHistoryRowsV1: vi.fn(async () => []),
  upsertDaaMarketPriceSnapshotsV1: vi.fn(async (rows: Array<Record<string, unknown>>) => rows.map((row) => ({
...
describe("market-cache-service-v1", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(listDaaMarketPriceSnapshotsV1).mockResolvedValue([]);
    vi.mocked(listLatestDaaMarketPriceHistoryRowsV1).mockResolvedValue([]);
  });
  ...
  it("快照状态为 stale 时即使较新也返回 stale", async () => {
    ...
    const result = await getMarketPricesWithCacheV1({
  ...
  it("429 后会单次重试并写入 fresh 快照", async () => {
    ...
    expect(vi.mocked(appendDaaMarketPriceHistoryRowsV1)).toHaveBeenCalledTimes(1);
  });
  ...
  it("刷新失败时会从 history 回捞最后成功价并标记 stale", async () => {
    ...
    const latestUpsertInput = vi.mocked(upsertDaaMarketPriceSnapshotsV1).mock.calls.at(-1)?.[0]?.[0] as Record<string, unknown> | undefined;
  ...
  it("禁用刷新时也会回捞 history 的最后成功价", async () => {
    ...
    expect(vi.mocked(upsertDaaMarketPriceSnapshotsV1)).not.toHaveBeenCalled();
  });
  ...
  it("无历史成功值且上游失败时返回 missing", async () => {
    ...
    expect(vi.mocked(listDaaMarketPriceSnapshotsV1)).toHaveBeenCalledTimes(1);
  });
});
