...
vi.mock("@/src/daa/modules/read/tradesReadServiceV1", () => ({
  buildTradesReadModelV1: vi.fn(),
}));
...
describe("trades-read-route-v1", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(buildTradesReadModelV1).mockResolvedValue({
      records: {
  ...
  it("返回统一 trades read model", async () => {
    ...
    expect(vi.mocked(buildTradesReadModelV1)).toHaveBeenCalledTimes(1);
  });
});
