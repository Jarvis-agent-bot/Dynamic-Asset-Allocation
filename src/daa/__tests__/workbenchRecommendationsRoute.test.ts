...
vi.mock("@/src/daa/modules/workbench/workbenchRecommendationService", () => ({
  runWorkbenchRecommendations: vi.fn(),
}));
...
describe("workbench-recommendations-route-v1", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(runWorkbenchRecommendations).mockResolvedValue({
  ...
  it("返回中文化建议结果", async () => {
    ...
    expect(vi.mocked(runWorkbenchRecommendations)).toHaveBeenCalledWith({ analysisFocus: "控制回撤并提升胜率" });
  });
});
