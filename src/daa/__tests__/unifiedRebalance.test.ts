...
function baseRequest(): DaaUnifiedRequest {
  return {
    account: { cash: 2000 },
    constraints: {
      minNotional: 100,
...
describe("unified-rebalance-v1", () => {
  it("强势+高分资产会提升触发阈值到 strong 配置", () => {
    const result = buildDaaUnifiedPlan(baseRequest());
  ...
  it("sb 标签资产会进入隔离舱并被降权至 0", () => {
    ...
    const result = buildDaaUnifiedPlan(req);
  ...
  it("symbol 级 targetWeights 会抛错（必须使用 assetKey）", () => {
    ...
    expect(() => buildDaaUnifiedPlan(req)).toThrow(/MARKET::SYMBOL/);
  });
});
