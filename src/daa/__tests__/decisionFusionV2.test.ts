...
import { MARKET_SCOPE_LABEL_ZH_V1 } from "@/src/daa/modules/marketContext/marketIndicatorCatalogV1";
import { fuseDecisionV2 } from "@/src/daa/modules/workbench/decisionFusionV2";
import type {
  DaaMarketContextV1,
...
import type { LlmDecisionOutputV2 } from "@/src/daa/llm/llmDecisionV2";
import type { RebalanceProposalV1 } from "@/src/daa/modules/workbench/workbenchTypesV1";

function makeIndicator(overrides: Partial<DaaMarketIndicatorSnapshotV1>): DaaMarketIndicatorSnapshotV1 {
  return {
    key: "vix",
    label: "VIX",
    category: "volatility",
...
function makeScopeContext(
  ...
  overrides: Partial<DaaMarketScopeContextV1> = {},
): DaaMarketScopeContextV1 {
  return {
    scope,
    label: MARKET_SCOPE_LABEL_ZH_V1[scope],
    generatedAt: "2026-03-01T00:00:00.000Z",
    regime: "transitional",
    riskOffScorePct: 50,
    confidencePct: 80,
...
function makeProposal(input: Partial<RebalanceProposalV1> & Pick<RebalanceProposalV1, "assetKey" | "symbol" | "side">): RebalanceProposalV1 {
  return {
...
function makeLlmDecision(overrides: Partial<LlmDecisionOutputV2> = {}): LlmDecisionOutputV2 {
  return {
    status: "skipped",
...
describe("decision-fusion-v2", () => {
  it("会按资产所属市场分别应用买入执行系数，并保留 SELL 不变", () => {
    ...
    const marketContext: DaaMarketContextV1 = {
      generatedAt: "2026-03-01T00:00:00.000Z",
      regime: "risk_off",
      riskOffScorePct: 80,
      confidencePct: 87,
