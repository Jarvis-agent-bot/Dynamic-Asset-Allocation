...
const mocks = vi.hoisted(() => ({
  ...
  listMarketIndicatorHistorySeries: vi.fn(),
}));
...
vi.mock("@/src/daa/modules/marketContext/marketIndicatorService", () => ({
  listMarketIndicatorHistorySeries: mocks.listMarketIndicatorHistorySeries,
}));
...
describe("market-indicator-history-devmem-fallback-v1", () => {
  ...
  it("dev:mem 下未鉴权也返回空历史序列，避免 overview 子面板刷 401", async () => {
    ...
    expect(mocks.listMarketIndicatorHistorySeries).not.toHaveBeenCalled();
  });
});
