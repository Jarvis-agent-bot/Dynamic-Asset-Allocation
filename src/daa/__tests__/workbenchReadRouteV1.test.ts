...
vi.mock("@/src/daa/modules/read/workbenchReadServiceV1", () => ({
  buildWorkbenchReadModelV1: vi.fn(),
}));
...
describe("workbench-read-route-v1", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(buildWorkbenchReadModelV1).mockResolvedValue({
      bootstrap: {
        baseCurrency: "USD",
  ...
  it("返回统一 workbench read model", async () => {
    ...
    expect(vi.mocked(buildWorkbenchReadModelV1)).toHaveBeenCalledTimes(1);
  });
});
