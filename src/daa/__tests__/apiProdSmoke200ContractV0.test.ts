...
describe("mainline DOD prod smoke contract", () => {
  it("keeps /api/daa/engine-health route wired for a successful health pass-through", () => {
    ...
    expect(route).toContain('const timeoutMs = parsePositiveIntEnv("DAA_ENGINE_TIMEOUT_MS", 10_000)');
    ...
    expect(route).toContain("return proxyToEngineJson({");
  ...
  it("keeps /daa/dashboard page entrypoint renderable (not redirect/notFound)", () => {
    ...
    expect(page).toContain('import DaaDashboardSkeleton from "./_components/DaaDashboardSkeleton"');
    ...
    expect(page).toContain("<Suspense fallback={<DaaDashboardSkeleton />}>");
