...
vi.mock("@/src/daa/modules/read/strategyLabSeedReadService", () => ({
  buildStrategyLabSeedReadModel: mocks.buildStrategyLabSeedReadModel,
}));
...
describe("strategy-lab-seed-route-devmem-fallback-v1", () => {
  ...
  it("seed 读接口在 dev:mem store 缺失时回退到空 seed model", async () => {
    mocks.buildStrategyLabSeedReadModel.mockRejectedValue(new Error('database "daa" does not exist'));
