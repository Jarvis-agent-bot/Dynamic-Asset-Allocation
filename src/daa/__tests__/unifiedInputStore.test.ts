...
import {
  bootstrapUnifiedInputRuntime,
  loadUnifiedInputState,
  writeUnifiedInputSlice,
...
describe("unifiedInputStore v1", () => {
  it("启动时返回默认内存态", () => {
    const st = loadUnifiedInputState();
  ...
  it("统一写入走内存状态，不依赖 localStorage", () => {
    ...
    writeUnifiedInputSlice("positions", [{ symbol: "SPY", market: "US", currency: "USD", qty: 2, price: 600, tags: [] }], { dispatchEvent: false });
    const st = loadUnifiedInputState();
  ...
  it("运行时引导只维护内存态，不触达浏览器存储", () => {
    expect(() => bootstrapUnifiedInputRuntime({ dispatchEvent: false })).not.toThrow();
  });
});
