/**
 * Briefing Presenter — 日报展示层测试
 *
 * 核心语义：TG 日报是"例外报告"。
 * - 无需人介入（无高严重度意外/无目标权重计划）→ 三行摘要（digest）
 * - 需要人介入 → 今日待办置顶的完整日报（full）
 * - 风险暴露 + 论点冲突合并为按资产聚合的"论点风险"
 * - 不输出纯遥测（论点/记忆计数、覆盖统计、样板文案）
 */
...
describe("digest 模式", () => {
  it("无待办时降级为三行摘要，不渲染完整板块", () => {
    const briefing = makeBriefing({
      surprises: [{ title: "中等意外", description: "描述", relatedThesisId: null, severityScore: 5, suggestedAction: "看看" }],
      cognitionGaps: [{ assetKey: "US::SGOV", portfolioWeight: 0, daysSinceLastInvestigation: 25, uncertaintyReason: "观察列表论点需要调查", suggestedInvestigation: "核对失效条件" }],
    });
    const html = formatBriefingForTelegram(briefing, { portfolio: mockPortfolio });

    expect(html).toContain("今日无需操作");
    expect(html).toContain("观察中：变化 1 条 · 论点复核 1 个");
    expect(html).toContain("总权益 <code>$99.9K</code>");
    // 完整板块不应出现
    expect(html).not.toContain("今日待办");
    expect(html).not.toContain("需要复核的变化");
    expect(html).not.toContain("中等意外");
    // 三行以内
...
describe("full 模式", () => {
  it(`severity >= ${BRIEFING_ACTION_SEVERITY} 的意外进入今日待办并触发完整日报`, () => {
    const briefing = makeBriefing({
    ...
    const briefing = makeBriefing({
      strategyOverlay: {
        ...
        targetAllocationPlan: {
          ...
          intents: [{ assetKey: "US::NVDA", symbol: "NVDA", proposedTargetWeightPct: 3, confidence: 86, reasoning: "论点证据转弱" }],
        },
      },
    });
...
describe("论点风险板块（按资产聚合）", () => {
  it("medium 及以上的失效影响与 high 冲突合并到同一资产行", () => {
    const briefing = makeBriefing({
      surprises: [{ title: "触发完整日报", description: "x", relatedThesisId: null, severityScore: 8, suggestedAction: "" }],
      ...
      thesisConflicts: [
        ...
        // medium severity 冲突不展示
        {
          thesisA: { id: "c", title: "次要论点甲", conviction: "medium" },
          thesisB: { id: "d", title: "次要论点乙", conviction: "low" },
          conflictType: "directional",
    ...
    expect(html).toContain("论点风险");
    // AAPL 一行同时聚合风险论点与冲突
    expect(html).toContain("[高] 苹果 AAPL 7.8% — 风险论点 1 个（最高「科技集中度 vs 利率上行」暴露 38.6%）；方向冲突 1 组");
    ...
    expect(html).not.toContain("次要论点甲");
  });
});
...
describe("遥测与样板文案清理", () => {
  it("不再输出论点/记忆计数 footer 和无计划样板文案", () => {
    const briefing = makeBriefing({
      surprises: [{ title: "触发完整日报", description: "x", relatedThesisId: null, severityScore: 8, suggestedAction: "" }],
      cognitionGaps: [{ assetKey: "US::NVDA", portfolioWeight: 0.05, daysSinceLastInvestigation: 10, uncertaintyReason: "需要调查", suggestedInvestigation: "" }],
    ...
    // 无实际动作时覆盖遥测整段省略
    expect(html).not.toContain("自动驾驶覆盖");
...
describe("formatBriefingForChat", () => {
  it("digest 模式输出摘要", () => {
    const text = formatBriefingForChat(makeBriefing({
      cognitionGaps: [{ assetKey: "US::SGOV", portfolioWeight: 0, daysSinceLastInvestigation: 25, uncertaintyReason: "需要调查", suggestedInvestigation: "" }],
    }));
    expect(text).toContain("今日无需操作");
    expect(text).toContain("论点复核 1 个");
