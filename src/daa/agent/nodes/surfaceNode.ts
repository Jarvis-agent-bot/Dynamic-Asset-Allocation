...
function computeDueForReview(
  ...
  watchlist: { candidates: Array<{ assetKey: string }> } | null,
): CognitionGap[] {
  ...
  for (const t of theses) {
    ...
    for (const assetKey of t.assetKeys) {
      ...
      const inWatchlist = watchlistKeys.has(assetKey);
      const uncertainMatch = t.conviction === "uncertain" && (weight > 0 || inWatchlist);
      const staleMatch = (weight > 0.05 || inWatchlist) && days > 7;
...
export async function surfaceNode(state: CognitiveState): Promise<CognitiveUpdate> {
  ...
  try {
    ...
    const dueForReview = computeDueForReview(theses, portfolio, state.watchlist ?? null);
