...
import { useAssetWorkbenchModel } from "../useAssetWorkbenchModel";
...
vi.mock("../useWorkbenchReadModel", () => ({
  useWorkbenchReadModel: (...args: unknown[]) => mockUseWorkbenchModel(...args),
}));

vi.mock("../asset-workbench/useAssetActions", () => ({
  useAssetActions: (...args: unknown[]) => mockUseWorkbenchAssetActions(...args),
}));

vi.mock("../asset-workbench/useRebalanceFlow", () => ({
  useRebalanceFlow: (...args: unknown[]) => mockUseWorkbenchRebalanceFlow(...args),
}));

vi.mock("../asset-workbench/useExecutionFlow", () => ({
...
describe("useAssetWorkbenchModel", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("优先使用 read model 的自洽汇总值", () => {
    ...
    const { result } = renderHook(() => useAssetWorkbenchModel());
