...
export type PortfolioWorkbenchTab = "positions" | "watchlist" | "analysis";

export function normalizePortfolioWorkbenchTab(input: string): PortfolioWorkbenchTab {
...
export function useWorkbenchReadModel(input: {
  initialTab?: string;
  syncPrices?: boolean;
  autoRiskCycle?: boolean;
} = {}) {
  const [activeTab, setActiveTab] = useState<PortfolioWorkbenchTab>(() => normalizePortfolioWorkbenchTab(String(input.initialTab || "")));
  const [workbenchModel, setWorkbenchModel] = useState<WorkbenchReadModel | null>(null);
  ...
  const bootstrap = workbenchModel?.bootstrap ?? null;
  const cycles = workbenchModel?.cycles ?? [];
  const snapshots = workbenchModel?.snapshots ?? [];
  const cashLedger = workbenchModel?.cashLedger ?? [];
  const signals = workbenchModel?.signals ?? [];
  const allocationSummary = workbenchModel?.allocationSummary ?? null;
  const equityDelta = workbenchModel?.equityDelta ?? null;
  const ledgerMeta = workbenchModel?.ledgerMeta ?? null;
  const notificationStatus = workbenchModel?.notificationStatus ?? null;

  const setCycles = useCallback<Dispatch<SetStateAction<RebalanceCycle[]>>>((nextValue) => {
    setWorkbenchModel((prev) => {
      if (!prev) return prev;
  ...
  useWorkbenchAutoRefresh(loadBootstrap);

  // --- SSE 实时价格流 ---
  const assetKeys = useMemo(() => {
    const rows = bootstrap?.assetUniverse || [];
    return rows.map((assetRow: { assetKey?: string }) => assetRow.assetKey || "").filter(Boolean);
  }, [bootstrap?.assetUniverse]);

  const priceStream = usePriceStream(assetKeys);

  // 将流式价格合并到 bootstrap（不修改原始 workbenchModel，仅在渲染层覆盖）
