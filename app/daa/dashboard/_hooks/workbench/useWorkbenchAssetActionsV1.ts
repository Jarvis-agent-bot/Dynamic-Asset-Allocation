...
import type { CalibrationDraftV1, OrderDraftV1 } from "@/app/daa/dashboard/_hooks/workbench/workbenchPageTypesV1";
import {
...
export function useWorkbenchAssetActionsV1(input: {
  bootstrap: WorkbenchBootstrapV1 | null;
  assetRows: AssetUniverseViewV1[];
  loading: boolean;
  busy: boolean;
  ...
  const [orderDraft, setOrderDraft] = useState<OrderDraftV1>(null);
  ...
  const [calibrationDraft, setCalibrationDraft] = useState<CalibrationDraftV1>(null);
  ...
  const handleAddManualOrder = useCallback(async (row: AssetUniverseViewV1, side: "BUY" | "SELL") => {
  ...
  const handlePreviewOrder = useCallback(async (payload: {
    ...
    qty?: number;
    notional?: number;
  }) => previewWorkbenchExecutionV1(payload), []);

  const handleSubmitManualOrder = useCallback(async (preview: WorkbenchMarketOrderPreviewResultV1) => {
  ...
  const handleSearchAssets = useCallback(async (payload: {
    ...
    assetClass: string;
    region: string;
  }): Promise<WorkbenchSearchAssetResultV1[]> => searchWorkbenchAssetsV1({
    q: payload.q,
  ...
  }): Promise<WorkbenchFeaturedAssetsResultV1> => listWorkbenchFeaturedAssetsV1({
    market: payload.market,
    assetClass: payload.assetClass,
    limitPerMarket: payload.limitPerMarket,
  }), []);

  const handleAddDiscoveredAsset = useCallback(async (item: WorkbenchSearchAssetResultV1 | WorkbenchFeaturedAssetItemV1) => {
  ...
  const handleToggleInlineInsights = useCallback(async (row: AssetUniverseViewV1) => {
  ...
  const handleRemoveFromWatchlist = useCallback(async (row: AssetUniverseViewV1) => {
  ...
  const handleToggleBasket = useCallback(async (row: AssetUniverseViewV1, nextInBasket: boolean) => {
    if (assetActioningKey) return;
    setAssetActioningKey(row.assetKey);
    try {
  ...
  const handleOpenCalibration = useCallback((row: AssetUniverseViewV1) => {
  ...
  const handleUpdateTargetWeight = useCallback(async (row: AssetUniverseViewV1, targetWeightPct: number) => {
