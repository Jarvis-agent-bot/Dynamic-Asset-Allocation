...
import { generateWorkbenchRebalanceCycleV1, patchWorkbenchRebalanceCycleV1 } from "@/src/daa/modules/workbench/workbenchApiV1";
import type {
  AssetUniverseViewV1,
  PreTradeRiskCheckV1,
  RebalanceCycleV1,
...
export function useWorkbenchRebalanceFlowV1(input: {
  ...
  riskCheck: PreTradeRiskCheckV1 | null;
  busy: boolean;
  setBusy: Dispatch<SetStateAction<boolean>>;
  loadBootstrap: (silent?: boolean, preferredCycleId?: string | null) => Promise<void>;
  syncCycleState: (nextCycle: RebalanceCycleV1 | null) => void;
}) {
  ...
  const isCurrentCycleTerminal = Boolean(input.currentCycle && isTerminalCycleStatusV1(input.currentCycle.status));
  ...
  const currentDecisionFacts = decisionMarketContext?.indicators.slice(0, 3).map((item) => (
    `${item.label} ${item.rawValue == null ? "N/A" : `${item.rawValue}${item.unit || ""}`} / ${marketPercentileTextV1(item.percentile252)}`
  )) || [];
  ...
  const handleToggleProposal = useCallback(async (assetKey: string, side: "BUY" | "SELL", selected: boolean) => {
    ...
    if (isTerminalCycleStatusV1(input.currentCycle.status)) {
    ...
    try {
      ...
      const next = await patchWorkbenchRebalanceCycleV1(input.currentCycle.cycleId, { selectedAssetSideKeys });
  ...
  const handleCancelCycle = useCallback(async () => {
    ...
    input.setBusy(true);
    try {
      const next = await patchWorkbenchRebalanceCycleV1(input.currentCycle.cycleId, {
