...
import { getStrategyLabSeedReadModelV1 } from "@/src/daa/modules/read/readApiV1";
import type { StrategyLabSeedReadModelV1 } from "@/src/daa/modules/read/readModelsV1";
...
function normalizeSelectedAssetKeysV1(rows: AssetUniverseViewV1[], preferred?: string[]): string[] {
...
function estimateCommonTradingBarsV1(input: {
  startDate: string;
  endDate: string;
  selectedAssets: AssetUniverseViewV1[];
}): number {
...
function buildPreflightChecksV1(input: {
  ...
  const estimatedBars = estimateCommonTradingBarsV1({
    startDate: input.startDate,
    endDate: input.endDate,
...
function describeExecutionGapV1(value: number) {
...
function describeRankShiftV1(value: number | null) {
...
function normalizeWarningAssetLabelV1(symbolRaw: string): string {
...
function normalizeMinOrderReasonLabelV1(reasonRaw: string): string {
...
function buildWarningSummaryV1(warnings: string[]): string[] {
  ...
  for (const raw of warnings) {
    ...
    if (roundedMatch) {
      ...
      const asset = normalizeWarningAssetLabelV1(roundedMatch[2]);
    ...
    if (suppressedMatch) {
      ...
      const asset = normalizeWarningAssetLabelV1(suppressedMatch[2]);
      const reason = normalizeMinOrderReasonLabelV1(suppressedMatch[3]);
      const key = `${asset}|${reason}`;
      const current = minOrderGroups.get(key) || { asset, reason, buy: 0, sell: 0 };
      if (side === "BUY") current.buy += 1;
      else current.sell += 1;
...
export function useStrategyLabModelV1() {
  const [seed, setSeed] = useState<StrategyLabSeedReadModelV1 | null>(null);
  ...
  const applySeedDefaults = useCallback((nextSeed: StrategyLabSeedReadModelV1) => {
    ...
    setSelectedAssetKeys((prev) => normalizeSelectedAssetKeysV1(nextSeed.availableAssets || [], prev.length > 0 ? prev : nextSeed.selectedAssetKeys));
    setResearchTargetWeightPctByAssetKey((prev) => {
      const base = buildResearchTargetWeightPctMapV1(nextSeed.availableAssets || []);
  ...
  const reloadSeed = useCallback(async (silent = false, preserveForm = true) => {
    ...
    setContextError("");
    try {
      const nextSeed = await getStrategyLabSeedReadModelV1();
      setSeed(nextSeed);
      if (!preserveForm || !hasHydratedDefaultsRef.current) {
        applySeedDefaults(nextSeed);
      } else {
        setSelectedAssetKeys((prev) => normalizeSelectedAssetKeysV1(nextSeed.availableAssets || [], prev));
        setResearchTargetWeightPctByAssetKey((prev) => {
          const base = buildResearchTargetWeightPctMapV1(nextSeed.availableAssets || []);
          return { ...base, ...prev };
        });
      ...
      const message = getApiErrorMessageV1(error);
  ...
  const selectedAssets = useMemo(() => {
    return sourceSelectedAssets.map((row) => ({
      ...row,
      targetWeightPct: clampWeightPctV1(researchTargetWeightPctByAssetKey[row.assetKey] ?? row.targetWeightPct ?? 0),
      targetWeightHint: clampWeightPctV1(researchTargetWeightPctByAssetKey[row.assetKey] ?? row.targetWeightHint ?? 0),
    }));
  ...
  const targetComparisonRows = useMemo(() => availableAssets.map((row) => ({
    ...
    researchTargetWeightPct: clampWeightPctV1(researchTargetWeightPctByAssetKey[row.assetKey] ?? row.targetWeightPct ?? 0),
  ...
  const selectedScenario = (scenarioMap.get(selectedScenarioId) || result?.scenarios?.[0] || null) as StrategyLabRunScenarioViewV1 | null;
  ...
  const selectedDiffRows = useMemo(
    () => selectedCandidate ? buildTargetWeightDiffRowsV1(result?.currentTargetWeights || {}, selectedCandidate.targetWeights || {}) : [],
  ...
  const preflightChecks = useMemo(() => buildPreflightChecksV1({
    selectedAssetCount,
  ...
  const executionGapMeta = useMemo(
    () => selectedCandidateScenarioComparison ? describeExecutionGapV1(selectedCandidateScenarioComparison.executionGap) : null,
    [selectedCandidateScenarioComparison],
  );
  const selectedCandidateRankMeta = useMemo(
    () => selectedCandidateScenarioComparison ? describeRankShiftV1(selectedCandidateScenarioComparison.rankShift) : null,
    [selectedCandidateScenarioComparison],
  );
...
export type StrategyLabModelV1 = ReturnType<typeof useStrategyLabModelV1>;
