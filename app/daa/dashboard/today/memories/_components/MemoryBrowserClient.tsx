    ...
    <div className="space-y-6">
          ...
          <button
            ...
            className={`rounded-full px-3 py-1 text-xs font-medium transition-colors ${
              typeFilter === t.value
                ...
                : "bg-[rgba(255,255,255,0.06)] text-[var(--muted)] hover:bg-[rgba(255,255,255,0.1)]"
            }`}
          >
            {t.label}
          </button>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {items.map(m => (
            <div key={m.id} className="group rounded-lg border border-[rgba(255,255,255,0.06)] bg-[rgba(255,255,255,0.02)] p-3 transition-colors hover:bg-[rgba(255,255,255,0.04)]">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className={`rounded px-1.5 py-0.5 text-[10px] font-medium ${TYPE_COLORS[m.memoryType] ?? "bg-[rgba(255,255,255,0.06)] text-[var(--faint)]"}`}>
                      {m.memoryType}
                    </span>
                  ...
                  {m.relevanceTags.length > 0 && (
                    <div className="mt-1.5 flex flex-wrap gap-1">
                      {m.relevanceTags.slice(0, 5).map(tag => (
                        <span key={tag} className="rounded bg-[rgba(255,255,255,0.04)] px-1.5 py-0.5 text-[10px] text-[var(--faint)]">{tag}</span>
                      ))}
                    </div>
                  )}
                </div>
              ...
              {/* 强度条 */}
              <div className="mt-2 h-1 rounded-full bg-[rgba(255,255,255,0.06)]">
      ...
      {pageCount > 1 && (
        <div className="flex items-center justify-center gap-3">
          <button
            ...
            disabled={offset === 0}
            className="flex items-center gap-1 rounded-lg px-3 py-1.5 text-xs text-[var(--muted)] hover:bg-[rgba(255,255,255,0.06)] disabled:opacity-30"
          >
