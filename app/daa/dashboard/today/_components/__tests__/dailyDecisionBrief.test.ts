...
import {
  ...
  buildDailyDecisionBriefFromBriefing,
  type DailyDecisionBriefInput,
} from "../dailyDecisionBrief";

function baseInput(overrides: Partial<DailyDecisionBriefInput> = {}): DailyDecisionBriefInput {
  return {
    queue: {
      decisionCount: 0,
      confirmCount: 0,
...
describe("dailyDecisionBrief", () => {
  it("只把目标权重变化算作需要人批准的动作", () => {
    const brief = buildDailyDecisionBrief(baseInput({
      queue: {
        decisionCount: 2,
        confirmCount: 3,
        investigateCount: 1,
  ...
  it("可以从 Agent briefing 直接生成给接口使用的摘要", () => {
    const brief = buildDailyDecisionBriefFromBriefing({
