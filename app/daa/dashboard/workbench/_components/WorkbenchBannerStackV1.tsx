...
export function WorkbenchBannerStackV1(props: {
  error: string;
      ...
      {props.executionReceipt ? (() => {
        ...
        return (
          <DeepLedgerNoticeBox
            tone={meta.tone}
            title={meta.title}
            description={`周期 ${props.executionReceipt.cycleId.slice(0, 8)} · 模式 ${props.executionReceipt.mode === "all" ? "执行全部" : "执行选中"} · ${formatDateTimeV1(props.executionReceipt.ts)}`}
          >
