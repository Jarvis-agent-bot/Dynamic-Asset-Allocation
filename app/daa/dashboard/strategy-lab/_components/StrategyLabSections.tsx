...
import { formatCurrency, formatDateRange, formatPercent } from "@/app/daa/dashboard/_components/daaFormatters";
...
import type { StrategyLabModel } from "@/app/daa/dashboard/_hooks/useStrategyLabModel";
import type { StrategyLabEnsembleConfig } from "@/src/daa/modules/strategyLab/strategyLabTypes";

const STRATEGY_STYLE_META_: Record<keyof StrategyLabEnsembleConfig, { label: string; description: string }> = {
...
function formatPercent01(value: number, digits = 2): string {
  return formatPercent((Number(value) || 0) * 100, digits);
}

function formatOptionalPercent01(value: number | null | undefined, digits = 2): string {
  if (value === null || value === undefined || !Number.isFinite(Number(value))) return "--";
  return formatPercent01(Number(value), digits);
}
...
function formatSharePct(value: number, digits = 0): string {
  return `${((Number(value) || 0) * 100).toFixed(digits)}%`;
}

function chartRows(model: StrategyLabModel) {
...
export function ResearchFrameBlock({ model }: { model: StrategyLabModel }) {
  return (
      ...
      {model.showResearchFrame ? (
        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          ...
          <label className="space-y-2">
            ...
            <select className={deepLedgerDenseFieldClassName} value={model.alignmentMode} onChange={(e) => model.setAlignmentMode(e.target.value as StrategyLabModel["alignmentMode"])}>
...
export function AdvancedSettingsBlock({ model }: { model: StrategyLabModel }) {
  return (
    <div className={deepLedgerSubtlePanelClassName + " p-4"}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
        ...
        <>
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            {model.normalizedEnsembleRows.map((row) => {
              const key = row.key as keyof StrategyLabEnsembleConfig;
              const meta = STRATEGY_STYLE_META_[key];
              return (
                <div key={row.key} className="rounded-[14px] border border-[var(--border)] bg-[rgba(8,12,20,0.55)] p-3">
                  ...
                  <div className="mt-2 flex items-center justify-between text-xs text-[var(--muted)]">
                    ...
                    <span>归一后 {formatSharePct(row.normalizedValue)}</span>
                  </div>
                </div>
              );
            })}
...
export function AssetPickerBlock({ model }: { model: StrategyLabModel }) {
  return (
    <div className={deepLedgerSubtlePanelClassName + " p-4"}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
...
export function StrategyLabSetupPanel({ model }: { model: StrategyLabModel }) {
  return (
    <>
      <DeepLedgerPageHeader
        title="策略实验室"
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <DeepLedgerMetricCard label="样本窗口" value={formatDateRange(model.startDate, model.endDate)} subLabel={`${model.alignmentMode === "intersection" ? "公共交易日" : "并集前值填充"} · 回看 ${model.lookbackBars} 个周期`} accent="green" />
      </div>
      ...
      <DeepLedgerPanel accent="cyan" title="先定义这轮实验" subtitle="先定义样本区间、执行约束和研究资产，保证本轮比较公平且可落地。">
        <div className="space-y-4">
          <ResearchFrameBlock model={model} />
          <AdvancedSettingsBlock model={model} />
          <AssetPickerBlock model={model} />
        </div>
      </DeepLedgerPanel>
    </>
  );
...
export function StrategyLabRunOverviewPanel({ model }: { model: StrategyLabModel }) {
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <DeepLedgerMiniStat label="总收益" value={formatPercent01(model.selectedCandidate?.backtest.metrics.totalReturn || 0)} hint={`vs ${model.result.benchmark.symbol} ${formatOptionalPercent01(model.selectedCandidate?.attribution.activeReturn)}`} tone="cyan" />
        <DeepLedgerMiniStat label="Sharpe / MDD" value={`${(model.selectedCandidate?.backtest.metrics.sharpe || 0).toFixed(2)} / ${formatPercent01(model.selectedCandidate?.backtest.metrics.maxDrawdown || 0)}`} hint={`胜率 ${formatPercent01(model.selectedCandidate?.backtest.metrics.winRate || 0)}`} tone="indigo" />
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {model.selectedScenario.candidates.map((candidate) => (
          ...
          >
            {candidate.label} · {formatPercent01(candidate.backtest.metrics.totalReturn)}
          </button>
        ))}
      </div>
    </DeepLedgerPanel>
...
export function StrategyLabCandidateDetailPanel({ model }: { model: StrategyLabModel }) {
      ...
      <div className="grid gap-4 xl:grid-cols-[0.92fr_1.08fr]">
        <div className="space-y-3">
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <DeepLedgerMiniStat label="主动收益" value={formatOptionalPercent01(model.selectedCandidate.attribution.activeReturn)} hint={`基准 ${model.result.benchmark.symbol}`} tone="cyan" />
            <DeepLedgerMiniStat label="波动 / Calmar" value={`${formatPercent01(model.selectedCandidate.attribution.metrics.volatility)} / ${model.selectedCandidate.attribution.metrics.calmar.toFixed(2)}`} hint={`MDD ${formatPercent01(model.selectedCandidate.attribution.metrics.maxDrawdown)}`} tone="amber" />
          </div>
            ...
            <tbody>
              {model.selectedDiffRows.map((row) => (
                <tr key={row.symbol} className="border-b border-[var(--border)]/70">
                  ...
                  <td className="px-4 py-3 text-[var(--muted)]">{formatPercent01(row.currentWeight)}</td>
                  <td className="px-4 py-3 text-[var(--muted)]">{formatPercent01(row.nextWeight)}</td>
                  <td className="px-4 py-3 text-[var(--text)]">{formatSignedPercent(row.deltaWeight * 100)}</td>
                </tr>
              ))}
...
export function StrategyLabDeepAnalysisPanel({ model }: { model: StrategyLabModel }) {
  if (!model.result || !model.selectedCandidate) return null;
  return (
    <DeepLedgerPanel
      accent="amber"
          ...
          <div className="space-y-4">
            <DeepLedgerNoticeBox tone="cyan" title="资产贡献" description="按收益贡献排序，先看收益来自哪些资产。">
              <div className="space-y-2">
                {model.topContributors.length > 0 ? model.topContributors.map((row: StrategyLabModel["topContributors"][number]) => (
                  <div key={row.symbol} className="flex items-center justify-between gap-3 rounded-[12px] border border-[var(--border)] bg-[rgba(8,12,20,0.55)] px-3 py-2">
                    <div>
                      ...
                      <div className="text-xs text-[var(--muted)]">平均权重 {formatPercent01(row.avgWeight)} · 资产收益 {formatPercent01(row.assetReturn)}</div>
                    </div>
                    <div className="text-sm font-semibold text-[var(--text)]">{formatPercent01(row.contributionToReturn)}</div>
                  </div>
                )) : <DashboardEmptyState title="暂无资产贡献" description="当前结果还没有贡献拆解，通常是样本不足或本轮结果尚未生成明细。" className="border-0 bg-transparent px-0 py-4 text-left" />}
              </div>
            </DeepLedgerNoticeBox>
                ...
                {model.selectedCandidate.attribution.rebalanceEvents.length > 0 ? model.selectedCandidate.attribution.rebalanceEvents.slice(0, 8).map((event) => (
                  <div key={`${event.date}-${event.turnover}`} className="flex items-center justify-between gap-3 rounded-[12px] border border-[var(--border)] bg-[rgba(8,12,20,0.55)] px-3 py-2">
                    <div>
                      ...
                      <div className="text-xs text-[var(--muted)]">调仓前最大偏离 {formatPercent01(event.driftBefore, 2)}</div>
                    </div>
...
export function StrategyLabEmptyStatePanel({ model }: { model: StrategyLabModel }) {
  return (
    <DeepLedgerEmptyState
      title="还没有回测结果"
