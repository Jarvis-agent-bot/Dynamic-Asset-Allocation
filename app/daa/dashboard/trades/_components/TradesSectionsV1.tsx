...
import type { TradesModelV1, TradeTabV1 } from "@/app/daa/dashboard/_hooks/useTradesModelV1";
...
const TAB_META: Record<TradeTabV1, { label: string; subtitle: string }> = {
...
export function TradesHeaderV1({ model }: { model: TradesModelV1 }) {
  return (
...
export function TradesSummaryMetricsV1({ model }: { model: TradesModelV1 }) {
  const activityLabel = model.latestActivityAt ? `最近活动 ${formatDateTimeV1(model.latestActivityAt)}` : "等待首个执行周期";
  return (
...
export function TradesErrorStateV1({ error }: { error: string }) {
  return <DashboardErrorNoticeV1 title="交易记录加载失败" description={error} />;
}

export function TradesTabsPanelV1({ model }: { model: TradesModelV1 }) {
  ...
  return (
    <DeepLedgerPanel accent="slate" title="视图切换" subtitle="周期是一级视角，订单与复盘是二级明细。">
      <div className="grid gap-2 rounded-2xl border border-[var(--border)] bg-[rgba(8,12,20,0.45)] p-2 sm:grid-cols-3">
        {(Object.keys(TAB_META) as TradeTabV1[]).map((tab) => {
      ...
      {model.activeTab === "cycles" ? <TradesCyclesPanelV1 model={model} /> : null}
      {model.activeTab === "orders" ? <TradesOrdersPanelV1 model={model} /> : null}
      {model.activeTab === "reports" ? <TradesReportsPanelV1 model={model} /> : null}
    </DeepLedgerPanel>
  );
}

export function TradesCyclesPanelV1({ model }: { model: TradesModelV1 }) {
        ...
        <tbody>
          {model.cycles.map((cycle) => (
            <tr key={cycle.cycleId}>
              ...
              <TableCellText align="right">{formatDateTimeV1(cycle.createdAt)}</TableCellText>
            </tr>
          ))}
        </tbody>
      </table>
...
export function TradesOrdersPanelV1({ model }: { model: TradesModelV1 }) {
        ...
        <tbody>
          {model.orders.map((order) => (
            <tr key={order.ticketId}>
              ...
              <TableCellText align="right">{formatDateTimeV1(order.updatedAt)}</TableCellText>
            </tr>
          ))}
        </tbody>
      </table>
...
export function TradesReportsPanelV1({ model }: { model: TradesModelV1 }) {
  return (
          ...
          <div key={report.cycleId} className="rounded-[18px] border border-[var(--border)] bg-[rgba(8,12,20,0.34)] p-4">
            ...
            >
              <div>
                ...
                <div className="mt-1 text-xs text-[var(--faint)]">{formatDateTimeV1(report.reportCreatedAt)}</div>
              </div>
