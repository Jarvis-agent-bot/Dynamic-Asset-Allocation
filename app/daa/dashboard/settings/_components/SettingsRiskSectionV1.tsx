...
export function SettingsRiskSectionV1(props: {
  ...
  return (
    <section id="settings-risk" className="scroll-mt-28">
      <SectionCardV1 title="风控参数">
        <div style={settingsGridCols3StyleV1}>
          {fields.map((field) => (
            <div key={field.label}>
              <FieldLabelV1>{field.label}</FieldLabelV1>
              <NumberInputV1 value={field.value} min={field.min} max={field.max} step={field.step} onChange={field.onChange} />
            </div>
          ))}
        </div>
