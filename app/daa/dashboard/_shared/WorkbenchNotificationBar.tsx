...
function executionReceiptMeta(status: ExecutionReceipt["status"]): {
  ...
  if (status === "success") return { title: "执行成功", tone: "success" };
  if (status === "submitted") return { title: "订单已提交", tone: "info" };
  if (status === "partial") return { title: "部分执行成功", tone: "warning" };
  if (status === "blocked") return { title: "执行被风控阻断", tone: "danger" };
  return { title: "执行失败", tone: "danger" };
}
...
export function WorkbenchNotificationBar(props: {
  ...
  const actionLinkClassName = "inline-flex items-center gap-2 rounded-[var(--radius-sm)] border border-[var(--border-strong)] bg-[var(--elevated)] px-3.5 py-2 text-sm font-medium text-[var(--muted)] transition-colors hover:border-[var(--primary)]/30 hover:text-[var(--text)]";

  // ─── 业务级告警标签（漂移已在调仓流程中展示，这里不再重复）───
  const alerts = useMemo(() => {
    const result: Array<{ key: string; tone: "warning" | "primary"; icon: React.ReactNode; label: string }> = [];
    ...
    // 数据健康
    const marketDataHealth = props.bootstrap?.marketDataHealth;
    if (marketDataHealth && marketDataHealth.status !== "ok") {
      result.push({
        key: "data",
        tone: "warning",
        icon: <Database className="h-3 w-3" />,
        label: marketDataHealth.status === "down" ? "市场数据不可用" : "市场数据已降级",
      });
    }
      ...
      {props.error ? (
        <DaaSurfaceNoticeBox
          ...
          action={props.authRequired ? (
            <Link href="/daa/login?returnTo=%2Fdaa%2Fdashboard%2Fportfolio" className="text-xs font-medium text-[var(--primary)] underline underline-offset-4">
              前往登录
            </Link>
          ) : null}
        />
        ...
        return (
          ...
          >
            ...
            {props.executionReceipt.reason ? (
              <div className="rounded-[var(--radius-md)] border border-dashed border-[var(--border-strong)] bg-[var(--surface)] px-3 py-2 text-xs text-[var(--faint)]">
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              ...
              <DaaSurfaceActionButton tone="neutral" onClick={props.onClearExecutionReceipt}>关闭回执</DaaSurfaceActionButton>
            </div>
          </DaaSurfaceNoticeBox>
        );
      })() : null}
