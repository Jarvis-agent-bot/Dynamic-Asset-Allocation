  ...
  return (
    <>
      {(() => {
        ...
        const wQatFormulaDriftPressureMode = wQatFormulaDriftAlertCount === 0
          ? 'stable-pressure'
          : wQatFormulaCriticalDriftCount > 0
            ? 'critical-pressure'
            : 'elevated-pressure';
        const wQatFormulaDriftReviewPriority = wQatFormulaDriftPressureMode === 'critical-pressure'
          ...
          : wQatFormulaDriftPressureMode === 'elevated-pressure'
            ? 'p2'
            : 'p3';
        const wQatFormulaDriftSlaLane = wQatFormulaDriftReviewPriority === 'p1'
          ? 'same-day-remediation'
          : wQatFormulaDriftReviewPriority === 'p2'
            ? 'next-day-review'
            : 'monitoring-backlog';
        const wQatFormulaDriftOwnerLane = wQatFormulaDriftReviewPriority === 'p1'
          ? 'formula-ops-oncall'
          : wQatFormulaDriftReviewPriority === 'p2'
        ...
        const wQatTimelinePressureMode = wQatTimelineCriticalCount === 0
          ...
          : wQatTimelineCriticalCount > 1
            ? 'critical-pressure'
            : 'elevated-pressure';
        const wQatTimelineReviewPriority = wQatTimelinePressureMode === 'critical-pressure'
          ? 'p1'
          : wQatTimelinePressureMode === 'elevated-pressure'
            ? 'p2'
            : 'p3';
        const wQatTimelineSlaLane = wQatTimelineReviewPriority === 'p1'
          ? 'same-day-remediation'
        ...
        const wQatEvidenceEscalationLane = wQatExplainabilityEvidenceReviewCount === 0
          ...
          : wQatEvidenceCriticalCount > 0
            ? 'critical-formula-remediation'
            : 'standard-formula-review';
        const wQatEvidencePressureMode = wQatExplainabilityEvidenceReviewCount === 0
          ? 'stable-pressure'
          : wQatEvidenceCriticalCount > 0
            ? 'critical-pressure'
            : 'elevated-pressure';
        const wQatEvidenceReviewPriority = wQatEvidencePressureMode === 'critical-pressure'
          ? 'p1'
          : wQatEvidencePressureMode === 'elevated-pressure'
            ? 'p2'
            : 'p3';
        const wQatEvidenceSlaLane = wQatEvidenceReviewPriority === 'p1'
          ? 'same-day-remediation'
        ...
        const factorTraceDriftPressureMode = driftAlertCount === 0
          ? 'stable-pressure'
          : factorTraceCriticalDriftCount > 0
            ? 'critical-pressure'
            : 'elevated-pressure';
        const factorTraceDriftReviewPriority = factorTraceDriftPressureMode === 'critical-pressure'
          ? 'p1'
        ...
        const factorTraceEvidencePressureMode = factorTraceEvidenceBlockedCount === 0
          ...
          : factorTraceEvidenceCriticalCount > 0
            ? 'critical-pressure'
            : 'elevated-pressure';
        const factorTraceEvidenceReviewPriority = factorTraceEvidencePressureMode === 'critical-pressure'
          ? 'p1'
          : factorTraceEvidencePressureMode === 'elevated-pressure'
            ? 'p2'
            : 'p3';
        const factorTraceEvidenceSlaLane = factorTraceEvidenceReviewPriority === 'p1'
          ? 'same-day-remediation'
          : factorTraceEvidenceReviewPriority === 'p2'
            ? 'next-day-review'
            : 'monitoring-backlog';
        const factorTraceEvidenceOwnerLane = factorTraceEvidenceReviewPriority === 'p1'
        ...
        const factorTraceTimelineReadinessPct = factorTraceEvidenceRows.length
          ? Math.round(((factorTraceEvidenceRows.length - factorTraceEvidenceBlockedCount) / factorTraceEvidenceRows.length) * 100)
          : 0;
        const factorTraceTimelineCriticalCount = factorTraceEvidenceRows.filter((row) => row.evidenceStatus === 'blocked').length;
        const factorTraceTimelineEscalationLane = factorTraceEvidenceBlockedCount === 0
          ? 'monitor-only'
          : factorTraceTimelineCriticalCount > 0
            ? 'critical-factor-timeline-remediation'
            : 'standard-factor-timeline-review';
        const factorTraceTimelinePressureMode = factorTraceEvidenceBlockedCount === 0
          ? 'stable-pressure'
          : factorTraceTimelineCriticalCount > 0
            ? 'critical-pressure'
            : 'elevated-pressure';
        const factorTraceTimelineReviewPriority = factorTraceTimelinePressureMode === 'critical-pressure'
          ? 'p1'
          : factorTraceTimelinePressureMode === 'elevated-pressure'
            ? 'p2'
            : 'p3';
        const factorTraceTimelineSlaLane = factorTraceTimelineReviewPriority === 'p1'
          ? 'same-day-remediation'
        ...
        return (
          <div style={{ marginTop: 8, padding: '10px 12px', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 12, background: 'rgba(0,0,0,0.1)' }}>
            ...
            {(() => {
              ...
              const factorPrecheckPressureMode = factorPrecheckBlockedCount === 0
                ? 'stable-pressure'
                : factorPrecheckCriticalCount > 0
                  ? 'critical-pressure'
                  : 'elevated-pressure';
              const factorPrecheckReviewPriority = factorPrecheckPressureMode === 'critical-pressure'
                ? 'p1'
                : factorPrecheckPressureMode === 'elevated-pressure'
                  ? 'p2'
                  : 'p3';
              const factorPrecheckSlaLane = factorPrecheckReviewPriority === 'p1'
                ? 'same-day-remediation'
            ...
            <div style={{ marginTop: 6, padding: '8px 10px', border: `1px dashed ${wQatFormulaDriftAlertCount > 0 ? 'rgba(245,158,11,0.55)' : 'rgba(34,197,94,0.55)'}`, borderRadius: 10, background: 'rgba(255,255,255,0.01)', fontSize: 11 }}>
              ...
              <div className="muted" style={{ marginTop: 4, display: 'grid', gap: 2, fontSize: 11 }}>
                ...
                <div>drift alert verdict: alerts=<b>{wQatFormulaDriftAlertCount}/{wQatFormulaDriftAlertRows.length}</b> · mode=<b>{wQatFormulaDriftAlertCount > 0 ? 'formula-drift-review-required' : 'formula-drift-stable'}</b> · readiness=<b>{wQatFormulaDriftReadinessPct}%</b> · route=<b>{wQatFormulaDriftRouteMode}</b> · critical alerts=<b>{wQatFormulaCriticalDriftCount}</b> · escalation lane=<b>{wQatFormulaDriftEscalationLane}</b> · pressure mode=<b>{wQatFormulaDriftPressureMode}</b> · review priority=<b>{wQatFormulaDriftReviewPriority}</b> · sla lane=<b>{wQatFormulaDriftSlaLane}</b> · owner lane=<b>{wQatFormulaDriftOwnerLane}</b></div>
          ...
          <div style={{ marginTop: 8, padding: '10px 12px', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 12, background: 'rgba(0,0,0,0.1)' }}>
            ...
            {(() => {
              ...
              const buyGatePrecheckReadinessPct = precheckRows.length
                ...
                : 0;
              const buyGatePrecheckCriticalCount = precheckRows.filter((row) => row.blockedGateCount >= 2).length;
              const buyGatePrecheckEscalationLane = readyRows === precheckRows.length
                ? 'monitor-only'
              ...
              const buyGatePrecheckPressureMode = readyRows === precheckRows.length
                ...
                : buyGatePrecheckCriticalCount > 0
                  ? 'critical-pressure'
                  : 'elevated-pressure';
              const buyGatePrecheckReviewPriority = buyGatePrecheckPressureMode === 'critical-pressure'
                ? 'p1'
                : buyGatePrecheckPressureMode === 'elevated-pressure'
                  ? 'p2'
                  : 'p3';
              const buyGatePrecheckSlaLane = buyGatePrecheckReviewPriority === 'p1'
                ? 'same-day-remediation'
              ...
              const buyGateDriftPressureMode = buyGateDriftAlertCount === 0
                ? 'stable-pressure'
                : buyGateCriticalDriftCount > 0
                  ? 'critical-pressure'
                  : 'elevated-pressure';
              const buyGateDriftReviewPriority = buyGateDriftPressureMode === 'critical-pressure'
                ? 'p1'
                : buyGateDriftPressureMode === 'elevated-pressure'
                  ? 'p2'
                  : 'p3';
              const buyGateDriftSlaLane = buyGateDriftReviewPriority === 'p1'
                ? 'same-day-remediation'
              ...
              const buyGateEvidenceEscalationLane = buyGateEvidenceReviewCount === 0
                ...
                : buyGateEvidenceCriticalCount > 0
                  ? 'critical-buy-gate-remediation'
                  : 'standard-buy-gate-review';
              const buyGateEvidencePressureMode = buyGateEvidenceReviewCount === 0
                ? 'stable-pressure'
                : buyGateEvidenceCriticalCount > 0
                  ? 'critical-pressure'
                  : 'elevated-pressure';
              const buyGateEvidenceReviewPriority = buyGateEvidencePressureMode === 'critical-pressure'
                ? 'p1'
              ...
              const buyGateEvidenceSlaLane = buyGateEvidenceReviewPriority === 'p1'
                ...
                : buyGateEvidenceReviewPriority === 'p2'
                  ? 'next-day-review'
                  : 'monitoring-backlog';
              const buyGateEvidenceOwnerLane = buyGateEvidenceReviewPriority === 'p1'
                ? 'buy-gate-ops-oncall'
                : buyGateEvidenceReviewPriority === 'p2'
                  ? 'buy-gate-review-queue'
                  : 'buy-gate-watchlist';
              const buyGateEvidenceReadinessPct = buyGateEvidenceTraceRows.length
                ...
                : 0;
              const buyGateEvidenceRouteMode = buyGateEvidenceReviewCount === 0
                ? 'buy-gate-evidence-clear-route'
                : buyGateEvidenceReviewCount === 1
                  ? 'buy-gate-evidence-review-route'
                  : 'buy-gate-evidence-remediation-route';
              const buyGateTimelineReadinessPct = buyGateEvidenceTraceRows.length
                ? Math.round(((buyGateEvidenceTraceRows.length - buyGateEvidenceReviewCount) / buyGateEvidenceTraceRows.length) * 100)
                : 0;
              const buyGateTimelineCriticalCount = buyGateEvidenceTraceRows.filter((row) => row.evidenceStatus === 'review-required').length;
              const buyGateTimelineEscalationLane = buyGateEvidenceReviewCount === 0
                ? 'monitor-only'
                : buyGateTimelineCriticalCount > 0
                  ? 'critical-buy-gate-timeline-remediation'
                  : 'standard-buy-gate-timeline-review';
              const buyGateTimelinePressureMode = buyGateEvidenceReviewCount === 0
                ? 'stable-pressure'
                : buyGateTimelineCriticalCount > 0
                  ? 'critical-pressure'
                  : 'elevated-pressure';
              const buyGateTimelineReviewPriority = buyGateTimelinePressureMode === 'critical-pressure'
                ? 'p1'
                : buyGateTimelinePressureMode === 'elevated-pressure'
                  ? 'p2'
                  : 'p3';
              const buyGateTimelineSlaLane = buyGateTimelineReviewPriority === 'p1'
                ? 'same-day-remediation'
              ...
              const buyGateTimelineRouteMode = buyGateEvidenceReviewCount === 0
                ...
                : buyGateEvidenceReviewCount === 1
                  ? 'buy-gate-timeline-review-route'
                  : 'buy-gate-timeline-remediation-route';
              const buyGateTimelineVerdict = buyGateEvidenceReviewCount > 0 ? 'buy-gate-precheck-review-required' : 'buy-gate-precheck-clear';
              ...
              return (
                <>
                  ...
                  <div style={{ marginTop: 6, padding: '8px 10px', border: '1px dashed rgba(255,255,255,0.2)', borderRadius: 10, background: 'rgba(255,255,255,0.01)', fontSize: 11 }}>
                    ...
                    <div className="muted" style={{ marginTop: 4, display: 'grid', gap: 2, fontSize: 11 }}>
                      <div>T0 precheck snapshot: rows=<b>{precheckRows.length}</b> · ready rows=<b>{readyRows}</b></div>
      ...
      {(() => {
        ...
        const guardrailDriftPressureMode = guardrailDriftAlertCount === 0
          ? 'stable-pressure'
          : guardrailCriticalDriftCount > 0
            ? 'critical-pressure'
            : 'elevated-pressure';
        const guardrailDriftReviewPriority = guardrailDriftPressureMode === 'critical-pressure'
          ...
          : guardrailDriftPressureMode === 'elevated-pressure'
            ? 'p2'
            : 'p3';
        const guardrailDriftSlaLane = guardrailDriftReviewPriority === 'p1'
          ? 'same-day-remediation'
        ...
        const guardrailEvidencePressureMode = guardrailEvidenceReviewCount === 0
          ...
          : guardrailEvidenceCriticalCount > 0
            ? 'critical-pressure'
            : 'elevated-pressure';
        const guardrailEvidenceReviewPriority = guardrailEvidencePressureMode === 'critical-pressure'
          ? 'p1'
          : guardrailEvidencePressureMode === 'elevated-pressure'
            ? 'p2'
            : 'p3';
        const guardrailEvidenceSlaLane = guardrailEvidenceReviewPriority === 'p1'
          ? 'same-day-remediation'
        ...
        const guardrailPrecheckPressureMode = guardrailPrecheckBlockedCount === 0
          ...
          : guardrailPrecheckCriticalCount > 1
            ? 'critical-pressure'
            : 'elevated-pressure';
        const guardrailPrecheckReviewPriority = guardrailPrecheckPressureMode === 'critical-pressure'
          ? 'p1'
        ...
        const guardrailTimelinePressureMode = guardrailEvidenceReviewCount === 0
          ? 'stable-pressure'
          : guardrailTimelineCriticalCount > 0
            ? 'critical-pressure'
            : 'elevated-pressure';
        const guardrailTimelineReviewPriority = guardrailTimelinePressureMode === 'critical-pressure'
          ? 'p1'
          : guardrailTimelinePressureMode === 'elevated-pressure'
            ? 'p2'
            : 'p3';
        const guardrailTimelineSlaLane = guardrailTimelineReviewPriority === 'p1'
          ? 'same-day-remediation'
        ...
        const manualPrecheckPressureMode = manualPrecheckBlockedCount === 0
          ...
          : manualPrecheckCriticalGateCount > 1
            ? 'critical-pressure'
            : 'elevated-pressure';
        const manualPrecheckReviewPriority = manualPrecheckPressureMode === 'critical-pressure'
          ? 'p1'
          : manualPrecheckPressureMode === 'elevated-pressure'
            ? 'p2'
            : 'p3';
        const manualPrecheckSlaLane = manualPrecheckReviewPriority === 'p1'
          ? 'same-day-remediation'
          : manualPrecheckReviewPriority === 'p2'
            ? 'next-day-review'
            : 'monitoring-backlog';
        ...
        const manualTimelinePressureMode = manualTimelineCriticalCount === 0
          ...
          : manualTimelineCriticalCount > 1
            ? 'critical-pressure'
            : 'elevated-pressure';
        const manualTimelineReviewPriority = manualTimelinePressureMode === 'critical-pressure'
          ? 'p1'
          : manualTimelinePressureMode === 'elevated-pressure'
            ? 'p2'
            : 'p3';
        const manualTimelineSlaLane = manualTimelineReviewPriority === 'p1'
          ? 'same-day-remediation'
          : manualTimelineReviewPriority === 'p2'
            ? 'next-day-review'
            : 'monitoring-backlog';
        ...
        const manualConfirmationDriftReadinessPct = manualConfirmationDriftAlertRows.length
          ...
          : 0;
        const manualConfirmationCriticalDriftCount = manualConfirmationDriftAlertRows.filter((row) => row.pressure === 'critical').length;
        const manualConfirmationDriftEscalationLane = manualConfirmationDriftAlertCount === 0
          ? 'monitor-only'
          : manualConfirmationCriticalDriftCount > 0
            ? 'critical-checkpoint-remediation'
            : 'standard-checkpoint-review';
        const manualConfirmationDriftPressureMode = manualConfirmationDriftAlertCount === 0
          ? 'stable-pressure'
          : manualConfirmationCriticalDriftCount > 0
            ? 'critical-pressure'
            : 'elevated-pressure';
        const manualConfirmationDriftReviewPriority = manualConfirmationDriftPressureMode === 'critical-pressure'
          ? 'p1'
          : manualConfirmationDriftPressureMode === 'elevated-pressure'
            ? 'p2'
            : 'p3';
        const manualConfirmationDriftSlaLane = manualConfirmationDriftReviewPriority === 'p1'
          ? 'same-day-remediation'
        ...
        const manualConfirmationEvidenceEscalationLane = manualConfirmationEvidenceReviewCount === 0
          ...
          : manualConfirmationEvidenceCriticalCount > 0
            ? 'critical-checkpoint-remediation'
            : 'standard-checkpoint-review';
        const manualConfirmationEvidencePressureMode = manualConfirmationEvidenceReviewCount === 0
          ? 'stable-pressure'
          : manualConfirmationEvidenceCriticalCount > 0
            ? 'critical-pressure'
            : 'elevated-pressure';
        const manualConfirmationEvidenceReviewPriority = manualConfirmationEvidencePressureMode === 'critical-pressure'
          ? 'p1'
          : manualConfirmationEvidencePressureMode === 'elevated-pressure'
            ? 'p2'
            : 'p3';
        const manualConfirmationEvidenceSlaLane = manualConfirmationEvidenceReviewPriority === 'p1'
          ? 'same-day-remediation'
          : manualConfirmationEvidenceReviewPriority === 'p2'
            ? 'next-day-review'
            : 'monitoring-backlog';
          ...
          <div style={{ marginTop: 8, padding: '10px 12px', border: `1px solid ${gate === 'pass' ? 'rgba(34,197,94,0.45)' : 'rgba(239,68,68,0.45)'}`, borderRadius: 12, background: gate === 'pass' ? 'rgba(22,163,74,0.08)' : 'rgba(220,38,38,0.08)' }}>
            ...
            <div style={{ marginTop: 6, padding: '8px 10px', border: `1px dashed ${manualCheckpointConfirmed ? 'rgba(34,197,94,0.55)' : 'rgba(245,158,11,0.55)'}`, borderRadius: 10, background: manualCheckpointConfirmed ? 'rgba(22,163,74,0.08)' : 'rgba(245,158,11,0.08)', fontSize: 11 }}>
              ...
              <div className="muted" style={{ marginTop: 4, display: 'grid', gap: 2, fontSize: 11 }}>
                ...
                <div>T5 timeline telemetry: readiness=<b>{manualPrecheckReadinessPct}%</b> · route=<b>{manualPrecheckRouteMode}</b> · critical gates=<b>{manualTimelineCriticalCount}</b> · escalation lane=<b>{manualTimelineEscalationLane}</b> · pressure mode=<b>{manualTimelinePressureMode}</b> · review priority=<b>{manualTimelineReviewPriority}</b> · sla lane=<b>{manualTimelineSlaLane}</b></div>
            ...
            <div style={{ marginTop: 6, padding: '8px 10px', border: `1px dashed ${manualConfirmationEvidenceReviewCount > 0 ? 'rgba(245,158,11,0.55)' : 'rgba(34,197,94,0.55)'}`, borderRadius: 10, background: 'rgba(255,255,255,0.01)', fontSize: 11 }}>
              ...
              <div className="muted" style={{ marginTop: 4, display: 'grid', gap: 2, fontSize: 11 }}>
                ...
                <div>evidence trace verdict: review rows=<b>{manualConfirmationEvidenceReviewCount}/{manualConfirmationEvidenceTraceRows.length}</b> · mode=<b>{manualConfirmationEvidenceReviewCount > 0 ? 'manual-confirmation-evidence-review-required' : 'manual-confirmation-evidence-clear'}</b> · readiness=<b>{manualConfirmationEvidenceReadinessPct}%</b> · route=<b>{manualConfirmationEvidenceRouteMode}</b> · critical rows=<b>{manualConfirmationEvidenceCriticalCount}</b> · escalation lane=<b>{manualConfirmationEvidenceEscalationLane}</b> · pressure mode=<b>{manualConfirmationEvidencePressureMode}</b> · review priority=<b>{manualConfirmationEvidenceReviewPriority}</b> · sla lane=<b>{manualConfirmationEvidenceSlaLane}</b></div>
