      ...
      {(() => {
        ...
        const guardrailPrecheckSlaLane = guardrailPrecheckReviewPriority === 'p1'
          ...
          : guardrailPrecheckReviewPriority === 'p2'
            ? 'next-day-review'
            : 'monitoring-backlog';
        const guardrailPrecheckOwnerLane = guardrailPrecheckReviewPriority === 'p1'
          ? 'guardrail-ops-oncall'
          : guardrailPrecheckReviewPriority === 'p2'
            ? 'guardrail-review-queue'
            : 'guardrail-watchlist';
        ...
        const manualTimelineSlaLane = manualTimelineReviewPriority === 'p1'
          ...
          : manualTimelineReviewPriority === 'p2'
            ? 'next-day-review'
            : 'monitoring-backlog';
        const manualTimelineOwnerLane = manualTimelineReviewPriority === 'p1'
          ? 'manual-ops-oncall'
          : manualTimelineReviewPriority === 'p2'
            ? 'manual-review-queue'
            : 'manual-watchlist';
        ...
        const manualConfirmationEvidenceSlaLane = manualConfirmationEvidenceReviewPriority === 'p1'
          ...
          : manualConfirmationEvidenceReviewPriority === 'p2'
            ? 'next-day-review'
            : 'monitoring-backlog';
        const manualConfirmationEvidenceOwnerLane = manualConfirmationEvidenceReviewPriority === 'p1'
          ? 'manual-ops-oncall'
          : manualConfirmationEvidenceReviewPriority === 'p2'
            ? 'manual-review-queue'
            : 'manual-watchlist';
          ...
          <div style={{ marginTop: 8, padding: '10px 12px', border: `1px solid ${gate === 'pass' ? 'rgba(34,197,94,0.45)' : 'rgba(239,68,68,0.45)'}`, borderRadius: 12, background: gate === 'pass' ? 'rgba(22,163,74,0.08)' : 'rgba(220,38,38,0.08)' }}>
            ...
            <div style={{ marginTop: 6, padding: '8px 10px', border: `1px dashed ${manualCheckpointConfirmed ? 'rgba(34,197,94,0.55)' : 'rgba(245,158,11,0.55)'}`, borderRadius: 10, background: manualCheckpointConfirmed ? 'rgba(22,163,74,0.08)' : 'rgba(245,158,11,0.08)', fontSize: 11 }}>
              ...
              <div className="muted" style={{ marginTop: 4, display: 'grid', gap: 2, fontSize: 11 }}>
                ...
                <div>T5 timeline telemetry: readiness=<b>{manualPrecheckReadinessPct}%</b> · route=<b>{manualPrecheckRouteMode}</b> · critical gates=<b>{manualTimelineCriticalCount}</b> · escalation lane=<b>{manualTimelineEscalationLane}</b> · pressure mode=<b>{manualTimelinePressureMode}</b> · review priority=<b>{manualTimelineReviewPriority}</b> · sla lane=<b>{manualTimelineSlaLane}</b> · owner lane=<b>{manualTimelineOwnerLane}</b></div>
