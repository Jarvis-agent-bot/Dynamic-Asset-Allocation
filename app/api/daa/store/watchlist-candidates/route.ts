...
import { listDaaWatchlistCandidatesV1, replaceDaaWatchlistCandidatesV1 } from "@/src/daa/store/daaStorePgV1";
...
export async function GET(req: Request) {
  return withApiHandlerV1(async () => {
    ...
    const candidates = await listDaaWatchlistCandidatesV1();
...
export async function POST(req: Request) {
  return withApiHandlerV1(async () => {
    ...
    const candidates = await replaceDaaWatchlistCandidatesV1(body.candidates as any[]);
    return okV1({ candidates });
  });
}
