...
export function clampLimit(rawLimit: string | null, opts?: { defaultValue?: number; max?: number }): number {
...
export function parseXueqiuCookie(raw: string): string {
...
export function getProviderErrorStatus(err: unknown, fallback: ProviderErrorStatus = 500): ProviderErrorStatus {
  if (err instanceof ProviderAdapterError) return err.status;
  return fallback;
}

export async function fetchTextWithTimeout(url: URL, init?: RequestInit, timeoutMs = 12000): Promise<Response> {
