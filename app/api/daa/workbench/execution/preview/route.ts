...
export async function POST(req: Request) {
  return withApiHandlerV1(async () => {
    ...
    let price = toPositive(row.lastPrice || row.holdingPrice);
    let priceSource = row.lastPrice > 0 ? "asset_universe.last_price" : "asset_universe.holding_price";
    let priceSnapshotAt = row.priceUpdatedAt || null;
    ...
    if (!(price > 0)) {
      ...
      price = latest.price;
      priceSource = `yfinance:${yfinanceSymbol}`;
      priceSnapshotAt = latest.ts;
    ...
    const fxRateForEstimate = hasFxRate ? Number(fxRateResolved) : 1;
    if (!hasFxRate && row.currency !== bootstrap.baseCurrency) {
      warnings.push(`缺少汇率 ${row.currency}/${bootstrap.baseCurrency}，基准币金额按 1:1 估算，执行时可能被拒绝`);
    }

    const notionalInBase = grossNotional * fxRateForEstimate;
    const feeInBase = fee * fxRateForEstimate;
    const totalCostInBase = side === "BUY" ? (notionalInBase + feeInBase) : (notionalInBase - feeInBase);

    if (side === "BUY" && hasFxRate && bootstrap.account.cash + 1e-9 < totalCostInBase) {
    ...
    if (side === "BUY" && !hasFxRate && row.currency !== bootstrap.baseCurrency) {
      warnings.push("由于缺少汇率，无法精确判断现金是否充足");
    }
    ...
    if (priceSnapshotAt) {
      ...
      if (Number.isFinite(ageMs) && ageMs > 6 * 60 * 60 * 1000) {
        warnings.push("价格快照超过 6 小时，建议先刷新行情再下单");
      }
    }
    ...
    if (!hasFxRate && row.currency !== bootstrap.baseCurrency) {
      warnings.push("仓位集中度按估算汇率计算，仅供参考");
    }
    if (nextWeightPct >= 30) {
